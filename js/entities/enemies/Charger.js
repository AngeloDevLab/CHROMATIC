...
export class Charger extends Enemy {
    ...
    _updatePatrol(dt) {
        ...
        if (this.knockbackTimer > 0) {
            // A hit (sword or the Prisma barrier, see Combat.js) knocks it
            // back but otherwise left `charging` true straight through the
            // stun - the instant it wore off, a still-standing player got
            // charged again with the cooldown never having actually started.
            // Ending it here covers every knockback source in one place
            // instead of every Combat.js hit-resolution function needing to
            // remember to do it.
            ...
            if (this.grounded && this._blockedAhead()) {
                ...
                // Mid-charge, no cooldown gate needed to keep going - only a
                // fresh charge (not already charging) is held back by it.
                const canStartOrContinue = this.charging || this.chargeCooldownTimer <= 0;
                this._setCharging(canStartOrContinue && this._canSeePlayer());
