...
/**
 * Per-level-group zones (LevelSession.js picks the right one off this by
 * levelNumber) - session decision on which tracks fit which levels, in the
 * order they should play. Lv1/Lv2 share a zone (so walking from one into
 * the other doesn't interrupt whatever's already playing), same for
 ...
 * zone's track list is currently active, crossfading into the next one
 * once the current one ends. setZone() no-ops if the requested zone is
 * already active, so moving between two screens that share one (Menu <->
 * Worldmap, Lv1 <-> Lv2, Lv4 <-> Lv5) never interrupts whatever's already
 * playing - it only restarts (from that zone's first track) when the zone
 * actually changes.
 */
export class MusicPlaylist {
    ...
    /**
     * main.js sets the initial zone before start() is ever called (so it's
     * already in place the instant start() does run) - gates setZone()
     * from trying to play anything before that, since playMusic() needs
     * the AudioContext-unlocking user gesture start() is itself gated
     * behind (LoadingState.js's "press any key/tap").
     */
    _started = false;

    /**
     * SoundManager.playMusic()'s crossfade stops the outgoing track via
     * source.stop(), which - same as a track reaching its end on its own -
     * fires that track's onended. Without this, a track cut short by a
     * zone switch still fires its *own* onEnded a fadeSeconds later, and
     * since index/trackKeys live on this shared instance, that stale
     * callback would advance/replay whatever zone is current by *then*,
     * not the one it was actually attached to - a self-sustaining cascade
     * of tracks cutting each other off almost immediately. Each
     * _playCurrentTrack() call bumps this; only its own onEnded closure's
     * captured value still matches by the time it fires - a stale one is
     ...
     * once start() has run (see _started above); before that, just records
     * which zone should play once it does.
     * @param {string} zone
     ...
     * that got cut short by a since-superseded zone switch (see
     * _generation above).
     */
    _playCurrentTrack() {
        this._generation += 1;
