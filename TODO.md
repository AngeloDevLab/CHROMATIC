...
- **Open bug: a dark edge/line is visible along an enemy's patrol path in Edge/Firefox** (reproduces even in the main menu's decorative Patroller pass, reliably after a single pass, hardware acceleration off doesn't fix it) - confirmed via direct pixel comparison it's not Chrome-only and not a simple compositing/accumulation bug: `ColorZone.overlayCtx`'s alpha stays 255 (fully opaque, matches the grey template) at the affected pixels, but the RGB is wrong (plain black instead of the template's actual color) exactly where the enemy's darken radius has passed. Extensive attempts to reproduce this via direct pixel-data testing (Playwright + real `msedge` channel, single pass, full real player-then-enemy cycle, many repeated passes, sampling at the patrol edge/turnaround points) all came back clean (no mismatch) - unable to root-cause it this session. Next things to try: compare Chrome vs Edge/Firefox `getImageData` output at the exact same reported world coordinate side by side (same page, same session), or suspect something GPU/driver-specific beyond just the hardware-acceleration toggle (already ruled that specific setting out).
