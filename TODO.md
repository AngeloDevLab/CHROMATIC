...
- Drop-Through-Platform (replaces Duck, see `docs/GDD/03-mechanics.md` 4.2) - `DROP_NUDGE_PX` (`player.js`) and the whole feel of it need real playtesting.
...
- Dash/jump/landing smoke VFX (`entities/vfx-effect.js`) playback fps (jump 24, landing 20, dash 28, `level-session.js`'s `VFX_CLIPS`) is first-guess - needs a look once played at real speed. Double Jump deliberately has no VFX (no ground contact to kick dust from).
