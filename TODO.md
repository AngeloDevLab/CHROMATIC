...
- AFK/idle animation (player idle after ~15s) - a checklist item from the reference project template, not automatically applicable since CHROMATIC deliberately diverged into its own concept/scope rather than following that template 1:1. Open design question, to be discussed/decided via a Loom recording rather than built reflexively.
