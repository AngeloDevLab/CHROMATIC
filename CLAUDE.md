...
- **File length**: max ~400 lines. A file that grows past this should have a cohesive chunk of it (e.g. health/damage state, the render pipeline) pulled into its own composed module rather than staying inline - see `Player.js`/`PlayerHealth.js`/`PlayerRenderer.js` for the pattern (Player.js keeps thin delegating methods/getters so external callers don't need to change).
...
**Entities** (`js/entities/`): `Entity` (base: position/size/velocity, `update`/`render`) → `Player`, and → `Enemy` → specific enemy types (`entities/enemies/`, e.g. Patroller/Charger/Shooter/Sentinel — not all implemented yet) → `Boss` → `Templateboss` (`entities/bosses/`). Player abilities (double jump, dash, wall jump) are composition, not inheritance — attached to `Player` as separate `Ability` instances, not subclasses. `Player`'s Health/Shield/buff bookkeeping (`PlayerHealth.js`) and sprite-drawing pipeline (`PlayerRenderer.js`) are likewise composed onto it rather than kept inline, per the file-length convention above — `Player.js` itself keeps thin delegating methods/getters (`health`, `shield`, `takeDamage()`, `visualTopY`, `render()`, etc.) so other files don't reach through the sub-objects directly.
