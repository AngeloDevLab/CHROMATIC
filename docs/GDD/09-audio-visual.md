...
Both fonts are self-hosted (`assets/fonts/`, loaded via `@font-face`), not pulled from a CDN, so the game works fully offline. Rendered through the HTML/CSS UI overlay (see [10-technical-architecture.md](10-technical-architecture.md) 11.8), not `fillText()` on the canvas - canvas text scaled up via `image-rendering: pixelated` would turn font edges blocky instead of staying crisp.
